...
func _load_data() -> void:
	...
	if saved_data != null:
		...
		_main.set_tempo_sound(_save.tempo_sound)
		_main.set_taches_sound(_save.taches_sound)
		_chrono.set_sound(_save.temps_sound) 
		_chrono.init_time()
...
func _on_SavePanel_save_pressed() -> void:
	...
	_save.temps_sound = _chrono.get_sound()
	_save.save_data()
	_chrono.init_time()
	_popup.start("Saved")
